...
parser.add_argument(
    "-loss_mode", choices=['bce', 'mse', 'mean', 'norm', 'l2', 'abs_mean', 'abs_l2'], default='l2')
...
parser.add_argument("-octave_mode", choices=[
                    'normal', 'advanced', 'manual_max', 'manual_min', 'manual'], default='normal')

# Channel options
parser.add_argument("-channels", type=str,
                    help="channels for DeepDream", default='-1')
parser.add_argument(
    "-channel_mode", choices=['all', 'strong', 'avg', 'weak', 'ignore'], default='all')
parser.add_argument("-channel_capture",
                    choices=['once', 'iter'], default='once')
...
parser.add_argument(
    "-zoom_mode", choices=['percent', 'pixel'], default='percent')
...
parser.add_argument("-image_capture_size",
                    help="Image size for initial capture, and classification", type=int, default=512)
...
parser.add_argument("-model_file", type=str,
                    default='models/bvlc_googlenet.pth')
parser.add_argument(
...
parser.add_argument("-random_transforms",
                    choices=['none', 'all', 'flip', 'rotate'], default='none')
parser.add_argument("-adjust_contrast", type=float,
                    help="try 99.98", default=-1)
parser.add_argument("-classify", type=int, default=0)

parser.add_argument("-dream_layers", help="layers for DeepDream",
                    default='inception_4d_3x3_reduce')
...
def main():
    ...
    params.model_type = auto_model_mode(
        params.model_file) if params.model_type == 'auto' else params.model_type
    input_mean = auto_mean(
        params.model_file, params.model_type) if params.model_mean == 'auto' else params.model_mean
    ...
    start_params = (dtype, params.random_transforms, params.jitter,
                    params.tv_weight, params.l2_weight, params.layer_sigma)
    primary_params = (params.loss_mode, params.dream_weight,
                      params.channels, params.channel_mode)
    ...
    if params.image_capture_size == -1:
        ...
        image_capture_size = tuple([int((float(params.image_capture_size) / max(
            base_img.size()))*x) for x in (base_img.size(2), base_img.size(3))])
        net_base(dream_image.resize_tensor(
            base_img.clone(), (image_capture_size)))
    ...
    if params.classify > 0:
        if params.image_capture_size == 0:
            ...
            feat = net_base(dream_image.resize_tensor(
                base_img.clone(), (image_capture_size)))
        classify_img(feat)
    ...
    if params.tile_size == 0:
        ...
        for iter in range(1, params.num_iterations+1):
            for octave, octave_sizes in enumerate(octave_list, 1):
                net = copy.deepcopy(
                    net_base) if not has_inception else net_base
                ...
                if img.size(2) != h or img.size(3) != w:
                    current_img = dream_image.resize_tensor(
                        img.clone(), (h, w))
                else:
            ...
            maybe_save(iter, current_img, content_image, input_mean,
                       output_start_num, params.leading_zeros)
            total_dream_losses, total_loss = [], [0]
        ...
        overlap_percent = params.overlap_percent / \
            100 if params.overlap_percent > 1 else params.overlap_percent
        ...
        print('Tile pattern: ' +
              str(tile_pattern[0]) + 'x' + str(tile_pattern[1]))

        octave_list = octave_calc(
            (tile_height, tile_width), params.octave_scale, params.num_octaves, params.octave_mode)
        print_octave_sizes(octave_list)
        ...
        for iter in range(1, params.num_iterations+1):
            for octave, octave_sizes in enumerate(octave_list, 1):
                ...
                for tile_num, tile_img in enumerate(img_tiles):
                    ...
                    def feval():
                        ...
                        maybe_print_tile_iter(
                            tile_num, num_tiles, num_calls[0], octave, params.octave_iter, dream_losses)
                ...
                maybe_save_octave(iter, octave, octave,
                                  tile_img, content_image, input_mean)
                octave_losses, tile_losses = [], []
...
def maybe_save(t, save_img, content_image, input_mean, start_num, leading_zeros):
    ...
    if should_save:
        ...
        save_output(t, save_img, content_image, "_" +
                    str(t+start_num).zfill(leading_zeros), input_mean, no_num)
...
def maybe_save_octave(t, n, o, save_img, content_image, input_mean):
    ...
    if should_save:
        save_output(t, save_img, content_image, "_" + str(t) +
                    "_" + str(o) + "_" + str(n), input_mean)
...
def maybe_print_octave_iter(t, n, total, dream_losses):
    if params.print_octave_iter > 0 and t % params.print_octave_iter == 0:
        print("Octave iter "+str(n) + " iteration " +
              str(t) + " / " + str(total))
        for i, loss_module in enumerate(dream_losses):
            print("  DeepDream " + str(i+1) + " loss: " +
...
def maybe_print_tile_iter(tile, num_tiles, t, n, total, dream_losses):
    if params.print_tile_iter > 0 and t % params.print_tile_iter == 0:
        print("Tile " + str(tile+1) + " / " + str(num_tiles) +
              " iteration " + str(t) + " / " + str(total))
        for i, loss_module in enumerate(dream_losses):
            print("  DeepDream " + str(i+1) + " loss: " +
                  str(loss_module.loss.item()))
    ...
    if not print_all_channels:
        for i, l in enumerate(dream_losses):
            if len(l.dream.channels) > 25:
                ch = l.dream.channels[0:25] + ['and ' +
                                               str(len(l.dream.channels[25:])) + ' more...']
            else:
                ch = l.dream.channels
            print('  ' + layers[i] + ': ', ch)
...
def setup_gpu():
    ...
    if "," in str(params.gpu):
        ...
        setup_cuda()
        dtype, backward_device = torch.cuda.FloatTensor, "cuda:" + \
            str(params.gpu)
    else:
        setup_cpu()
...
def setup_multi_device(net_base):
    ...
    new_net_base = ModelParallel(
        net_base, params.gpu, params.multidevice_strategy)
    return new_net_base
...
def deprocess(output_tensor, mode='caffe', input_mean=[-103.939, -116.779, -123.68]):
    ...
    if mode == 'caffe':
        ...
        bgr2rgb = transforms.Compose(
            [transforms.Lambda(lambda x: x[torch.LongTensor([2, 1, 0])])])
        output_tensor = bgr2rgb(
            Normalize(output_tensor.squeeze(0).cpu())) / 256
    elif mode == 'pytorch':
        Normalize = transforms.Compose(
            [transforms.Normalize(mean=input_mean, std=[1, 1, 1])])
...
def print_octave_sizes(octave_list):
    print('\nPerforming ' + str(len(octave_list)) +
...
def octave_calc(image_size, octave_scale, num_octaves, mode='normal'):
    ...
    if len(octave_scale.split(',')) == 1 and 'manual' not in mode:
        ...
        if mode == 'manual':
            octave_scale = [octave_scale[o:o+2]
                            for o in range(0, len(octave_scale), 2)]
    ...
    if mode == 'manual_max' or mode == 'manual_min':
        ...
        assert len(octave_scale) + 1 == num_octaves, \
            "Exected " + str(num_octaves - 1) + " octave sizes, but got " + \
            str(len(octave_scale)) + " containing: " + str(octave_scale)
    ...
    if mode == 'normal':
        ...
        for o in octave_scale:
            new_size = tuple([int((float(o) / max(image_size))*x)
                             for x in (h_size, w_size)])
            octave_list.append(new_size)
    elif mode == 'manual_min':
        for o in octave_scale:
            new_size = tuple([int((float(o) / min(image_size))*x)
                             for x in (h_size, w_size)])
            octave_list.append(new_size)
    elif mode == 'manual':
        for o_size in octave_scale:
            assert len(
                o_size) % 2 == 0, "Manual octave sizes must be in pairs like: Height,Width,Height,Width..."
